...
func move_state(delta: float) -> void:
	...
	if(Input.is_action_just_pressed("jump")):
		playback.travel("JumpState")

	velocity = input_vector * SPEED
	move_and_slide()

func jump_state(delta: float) -> void:
	velocity = last_input_vector * SPEED/2
	
	move_and_slide()

func update_blend_position(direction_vector: Vector2) -> void:
	animation_tree.set("parameters/StateMachine/MoveState/WalkingState/blend_position", direction_vector)
	...
	animation_tree.set("parameters/StateMachine/JumpState/blend_position", direction_vector)
